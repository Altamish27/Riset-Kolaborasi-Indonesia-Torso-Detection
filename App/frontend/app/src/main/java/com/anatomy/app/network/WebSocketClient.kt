...
class ChatWebSocketClient(private val httpClient: OkHttpClient) {
    ...
    fun disconnect() {
        try {
            ...
            Log.e(TAG, "Error during chat disconnect", e)
        }
    }
}
...
class VoiceWebSocketClient(private val httpClient: OkHttpClient) {
    ...
    fun sendJson(message: Map<String, String>): Boolean {
        try {
            if (webSocket == null) {
                Log.e(TAG, "WebSocket is null, cannot send JSON message")
                return false
            }
    ...
    fun sendAudio(audioData: ByteArray): Boolean {
        try {
            if (webSocket == null) {
                Log.e(TAG, "WebSocket is null, cannot send audio")
                return false
            }
    ...
    fun disconnect() {
        try {
            ...
            Log.e(TAG, "Error during voice disconnect", e)
        }
    }
}
